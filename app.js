...
const DEFAULT_MEMBERS = [
  { id: 'rajesh', name: 'Rajesh', role: 'Father', avatar: 'R' },
  { id: 'bindu', name: 'Bindu', role: 'Mother', avatar: 'B' },
  { id: 'arun', name: 'Arun', role: 'Son', avatar: 'A' }
];
...
const SEED_EXPENSES = [
  ...
  },
  {
    ...
    member: 'bindu',
    description: 'Supermarket grocery sadhanangal'
  },
  {
    id: 'seed-3',
    amount: 350,
  ...
  },
  {
    ...
    member: 'rajesh',
    description: 'KSEB electric bill'
  },
  {
    id: 'seed-5',
    amount: 5000,
    ...
    member: 'arun',
    description: 'College fees term 1'
  },
  {
    id: 'seed-6',
    amount: 800,
    category: 'medical',
    date: getFormattedDate(-10), // 10 days ago
    member: 'bindu',
    description: 'Doctor checkup tablets'
  },
  {
    id: 'seed-7',
    amount: 2500,
    category: 'shopping',
    date: getFormattedDate(-15), // 15 days ago
    member: 'bindu',
    description: 'Onam dress shopping'
  },
  {
    id: 'seed-8',
    amount: 8000,
    category: 'rent',
    date: getFormattedDate(-18), // 18 days ago
    member: 'rajesh',
...
function initAppState() {
  const storedState = localStorage.getItem('kudumbam_expense_state');
  if (storedState) {
    try {
      appState = JSON.parse(storedState);
      // Backwards compatibility fixes
      if (!appState.categories || appState.categories.length === 0) appState.categories = DEFAULT_CATEGORIES;
      if (!appState.members || appState.members.length === 0) appState.members = DEFAULT_MEMBERS;
      if (!appState.activeMemberId) appState.activeMemberId = appState.members[0].id;
    } catch (e) {
      console.error('Failed to parse storage state, using defaults', e);
      loadDefaults();
    }
  } else {
    loadDefaults();
...
function loadDefaults() {
  appState.budget = 30000;
  appState.expenses = [...SEED_EXPENSES];
  appState.categories = [...DEFAULT_CATEGORIES];
  appState.members = [...DEFAULT_MEMBERS];
  appState.activeMemberId = 'rajesh';
  appState.pinLock = { enabled: false, pin: '' };
}

function saveStateToStorage() {
  localStorage.setItem('kudumbam_expense_state', JSON.stringify(appState));
}
...
function handleDeleteCategory(id) {
  if (confirm('Are you sure you want to delete this custom category?')) {
    ...
    // Relabel previous transactions with deleted category to 'other'
    appState.expenses.forEach(exp => {
      if (exp.category === id) exp.category = 'other';
    });
    saveStateToStorage();
...
function importData(e) {
  ...
  reader.onload = function(event) {
    try {
      ...
      if (importedState.expenses && importedState.budget && importedState.categories) {
...
function resetData() {
  if (confirm('⚠️ WARNING: This will permanently erase ALL logged transactions, budget limits, custom categories, and family members. Are you sure you want to proceed?')) {
    localStorage.removeItem('kudumbam_expense_state');
    loadDefaults();
    saveStateToStorage();
    populateFormSelectors();
...
function registerEventListeners() {
  ...
  // Preview form submit
  dom.expenseEditorForm.addEventListener('submit', (e) => {
    e.preventDefault();
    
    const editId = dom.editExpenseId.value;
    const expenseData = {
      id: editId || 'exp-' + Date.now() + '-' + Math.round(Math.random() * 1000),
